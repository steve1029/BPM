...
function get_cylindrical_profile(
    x::AbstractVector,
    y::AbstractVector,
    z::AbstractVector,
    nclad::Number,
...
function get_S_bend_profile(
    ...
    xc = Int(ceil(Nx/2))
    yc = Int(ceil(Ny/2))
    xm = xc + Int(round(-xwidth/2/dx))
    xp = xc + Int(round(+xwidth/2/dx))
    ym = yc + Int(round(-ywidth/2/dy))
    yp = yc + Int(round(+ywidth/2/dy))
...
function get_Efield(
    ...
    for step in 2:Nz
        adi = Efield[:,:,step-1]
        adi = _1st_sub_step_scalar(step, λ, dy, dz, nref, α, n, Py, Qy, Ry, adi;)
        adi = _2nd_sub_step_scalar(step, λ, dx, dz, nref, α, n, Fx, Gx, Hx, adi;)
        Efield[:,:,step] = adi
    end
...
function _pml(
    ...
    println("x-pml thick = $(bdwx/um) um")
    println("y-pml thick = $(bdwy/um) um")
...
function _1st_sub_step_scalar(
    ...
    for i in 1:Nx
        ...
        aj = (-α*P[i,:,step] ./ (dy^2))
        bj = (2im*k0*nref/dz) .+ (α*R[i,:,step] ./ (dy^2)) .- (α*0.5*(k0^2) .* nd)
        cj = (-α*Q[i,:,step] ./ (dy^2))
        Aj = diagm(-1=>aj, 0=>bj, 1=>cj)

        Dj = (2im*k0*nref/dz) .- ((1-α)*R[i,:,step] ./ (dy^2)) .+ ((1-α)*0.5*(k0^2) .* nd)
        above = ((1-α)/(dy^2)) * Q[i,:,step]
        below = ((1-α)/(dy^2)) * P[i,:,step]
...
function _2nd_sub_step_scalar(
    ...
    for j in 1:Ny
        ...
        ai = (-α/dx^2) .* F[:,j,step]
        bi = (2im*k0*nref/dz) .+ ((α/dx^2) .* H[:,j,step]) .- (α*0.5*k0^2 .* nd)
        ci = (-α/dx^2) .* G[:,j,step]
        Ai = diagm(-1=>ai, 0=>bi, 1=>ci)

        Di = (2im*k0*nref/dz) .- ((1-α)/dx^2 .* H[:,j,step]) .+ ((1-α)*0.5*k0^2 .* nd)
        above = ((1-α) / dx^2) .* G[:,j,step]
        below = ((1-α) / dx^2) .* F[:,j,step]
...
function serial_plot_field(
    ...
    for intercept in intercepts
        intstr = @sprintf("%5.1f", intercept/um)
        pp = plot_field(x, y, z, field, which_plane, intercept; clim=clim)
        savefig(pp, working_dir*"$nametag-xy-($intstr)μm-profile.png")
    end
